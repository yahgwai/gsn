...
contract IRelayHub {
    ...
    struct RelayData {
        address senderAccount; // the sender (signer) of this message
        uint256 senderNonce;  // sender’s nonce on the RelayHub
        address gasRecipient; // the contract that handles the relay payment
        address relayAddress;  // actual relay to make the transaction
        uint256 pctRelayFee;  // multiplier (described as %) on actual transaction price
        uint256 flatRelayFee; // added flat fee to the tx (wei)
        bytes signature;
        bytes approvalData;
    }
